# -*- coding: utf-8 -*-
"""
Created on Sat Jun  3 07:57:40 2017
...
#plt.style.use('nbagg')
plt.style.use('ggplot') # use "ggplot" style for grap
...
for i in range(nums): # repeat the following operations from i=0 to nums-1
    W = std*np.random.randn(nump,dim) # generate an array of random forces accordingly V = V*(1-zeta/m*dt)+W/m # update velocity via Eq.(F9)
